...
const pagination = computed<PaginationData>(() => ({
    current_page: props.programs.meta.current_page,
    last_page: props.programs.meta.last_page,
    per_page: props.programs.meta.per_page,
    total: props.programs.meta.total,
}));

const getFilterParams = () => ({
    search: search.value || undefined,
    status: statusFilter.value !== 'all' ? statusFilter.value : undefined,
    degree_level: degreeLevelFilter.value !== 'all' ? degreeLevelFilter.value : undefined,
...
const handleSearch = () => {
    router.get('/dashboard/programs', getFilterParams(), { preserveState: true });
};

watch([statusFilter, degreeLevelFilter, schoolFilter], () => {
    router.get('/dashboard/programs', getFilterParams(), { preserveState: true });
});
                    ...
                    <template #cell-name="{ item }">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div v-if="item.department_name" class="text-xs text-muted-foreground">{{ item.department_name }}</div>
                            </div>
                        </div>
                    </template>
                    <template #cell-degree_level="{ item }">
                        <Badge variant="outline">{{ item.degree_level_label }}</Badge>
                    </template>
