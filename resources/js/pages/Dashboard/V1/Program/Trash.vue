...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Trash', href: '/dashboard/programs/trash' },
];

const trashConfig: TrashConfigLocal = {
    entityLabel: 'Program',
    entityLabelPlural: 'Programs',
    restoreRoute: (uuid: string) => `/dashboard/programs/${uuid}/restore`,
    forceDeleteRoute: (uuid: string) => `/dashboard/programs/${uuid}/force-delete`,
    listRoute: '/dashboard/programs/trash',
};
...
const handlePageChange = (page: number) => {
    router.get('/dashboard/programs/trash', {
        page,
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        <Head title="Programs Trash" />

        <div class="flex h-full flex-1 flex-col gap-6 p-6">
            <!-- Header -->
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold">Programs Trash</h1>
                    <p class="text-sm text-muted-foreground">
                        Manage deleted programs - restore or permanently delete
                    </p>
                </div>
                <ButtonGroup>
            ...
            <TrashTable
                ...
                :show-type="false"
                empty-message="No deleted programs found."
                empty-trash-route="/dashboard/programs/trash/empty"
